...
fn format_command_list(output: &mut String, children: &BTreeMap<String, CommandTree>, indent: usize) {
    ...
    for (name, child) in children {
        ...
        match child {
            CommandTree::Leaf(cmd) => {
                ...
                output.push_str(&format!("{}{:<width$}{}\n", indent_str, name, description, width = max_width));
            }
            CommandTree::Group { description, children: group_children, .. } => {
                let desc = description.as_deref().unwrap_or("");
                output.push_str(&format!("{}{:<width$}{}\n", indent_str, name, desc, width = max_width));
    ...
    if !cmd.args.is_empty() {
        ...
        for arg in &cmd.args {
            ...
            if let Some(choices) = &arg.choices {
                output.push_str(&format!("                  choices: {}\n", choices.join(", ")));
            }
    ...
    if !children.is_empty() {
        ...
        for child_name in ordered_keys {
            if let Some(child) = children.get(child_name) {
                ...
                output.push_str(&format!("  {:<width$}{}\n", child_name, desc, width = max_width));
            }
        }
    }
...
pub fn print_group_help(
    ...
    print!("{}", format_group_help(name, group_name, description, children, order));
}
...
mod tests {
    ...
    fn test_format_top_level_help_includes_commands() {
        ...
        tree.insert("deploy", CommandTree::Leaf(CommandDef {
            name: "deploy".into(),
    ...
    fn test_format_top_level_help_shows_nested_groups() {
        ...
        if let CommandTree::Group { children, .. } = &mut db_group {
            children.insert("migrate".into(), CommandTree::Leaf(CommandDef {
                name: "migrate".into(),
    ...
    fn test_format_group_help() {
        ...
        children.insert("seed".into(), CommandTree::Leaf(CommandDef {
            name: "seed".into(),
    ...
    fn test_format_group_help_respects_order() {
        let mut children = BTreeMap::new();
        children.insert("alpha".into(), CommandTree::Leaf(CommandDef {
            name: "alpha".into(),
        ...
        children.insert("zebra".into(), CommandTree::Leaf(CommandDef {
            name: "zebra".into(),
